    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div>
                    <?php 
                    // Check if all necessary GET parameters are set for exporting
                    if (isset($_GET['secid']) && isset($_GET['semester']) && isset($_GET['course']) && isset($_GET['year'])) {
                        ...
                        <!-- Form for exporting class schedule to CSV -->
                        ...
                        <form method="post" action="export_fees.php?secid=<?php echo $secid; ?>&semester=<?php echo $semester; ?>&year=<?php echo $year; ?>&course=<?php echo $course; ?>" align="center">  
                            <input type="submit" name="export_fees" value="Export Fees" class="btn btn-info" />  
                        </form>  
                        ...
                        <div class="card">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover" id="insloadtable">
                                    ...
                                    // Retrieve schedule based on selected course and semester
                                    ...
                                    if ($loads) {
                                        while ($lrow = $loads->fetch_assoc()) {
                                            // Fetch data for each schedule entry
                    ...
                    <?php } // End of checking GET parameters ?>
                </div>
            </div>
        </div>
    </div>
...
<script>
    // JavaScript to handle modal display if needed
    $(document).ready(function(){
        // Show modal with specific data when a row is clicked (if desired)
        $('#insloadtable tbody').on('click', 'tr', function () {
            const row = $(this);
            const data = {
                schedule: row.find('td').eq(0).text(),
                time: row.find('td').eq(0).text(),
                courseCode: row.find('td').eq(2).text(),
                description: row.find('td').eq(3).text(),
                units: row.find('td').eq(4).text(),
                room: row.find('td').eq(5).text(),
                instructor: row.find('td').eq(6).text(),
            };

            $('.scheddays span').text(data.schedule);
            $('.time span').text(data.time);
            $('.course span').text(data.courseCode);
            $('.description span').text(data.description);
            $('.units span').text(data.units);
            $('.room span').text(data.room);
            $('.instructor span').text(data.instructor);
            $('#myModal').modal('show');
        });
    });
</script>
