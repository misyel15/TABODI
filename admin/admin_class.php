...
class Action {
    ...
    if (empty($id)) {
        ...
        // Insert notification for new course
        if ($save) {
            $message = "New course '$course' has been added.";
            $this->db->query("INSERT INTO notifications (dept_id, message) VALUES ('$dept_id', '$message')");
        }
    } else {
        // Update existing course
        $save = $this->db->query("UPDATE courses SET $data WHERE id = $id");

        // Insert notification for updated course
        if ($save) {
            $message = "Course '$course' has been updated.";
            $this->db->query("INSERT INTO notifications (dept_id, message) VALUES ('$dept_id', '$message')");
        }
    }
...
function delete_course() {
    ...
    // Get the course details before deleting (for the notification message)
    $course = $this->db->query("SELECT * FROM courses WHERE id = $id")->fetch_assoc();

    // Delete the course
    $delete = $this->db->query("DELETE FROM courses WHERE id = ".$id);

    // Insert notification for deleted course
    if ($delete) {
        $message = "Course '{$course['course']}' has been deleted.";
        $this->db->query("INSERT INTO notifications (dept_id, message) VALUES ('{$course['dept_id']}', '$message')");
        return 1;
    }
